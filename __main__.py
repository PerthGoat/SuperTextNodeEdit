...
class RTFWindow:
    ...
    def clipboardHyperlinkTarget(self, link):
        """Return a destination that other applications can open."""
        kind = link.get('kind', 'url')
        target = link.get('target', '')
        if kind == 'node':
            note_path = os.path.join(
                self._node_root_path(),
                target.replace('/', os.sep) + '.rtf',
            )
            return Path(note_path).resolve().as_uri()
        if kind == 'file' and not re.match(
            r'^[A-Za-z][A-Za-z0-9+.-]*://',
            target,
        ):
    ...
    def selectionAsHTML(self, dumped_text, start):
        """Convert a text-widget selection to link-aware clipboard HTML."""
        active_padding_tags = []
        active_hyperlink_tags = [
            tag for tag in self.text.tag_names(start)
            if tag in self.hyperlink_tags
        ]
        parts = ['<span style="white-space: pre-wrap;">']

        def open_link(tag):
            target = self.clipboardHyperlinkTarget(self.hyperlink_tags[tag])
            parts.append(f'<a href="{html.escape(target, quote=True)}">')
        ...
        for token_type, token_value, _ in dumped_text:
            ...
            if token_type == 'tagon' and token_value in self.hyperlink_tags:
                if token_value not in active_hyperlink_tags:
                    parts.extend('</a>' for _ in active_hyperlink_tags)
                    active_hyperlink_tags.clear()
                    open_link(token_value)
            ...
            if token_type == 'text' and not active_padding_tags:
                parts.append(html.escape(token_value))
    ...
    def _displayNestedRTFStructure(self, structure, style, insertion_index='end'):
        ...
        # Field instructions are hidden metadata. Clipboard RTF generated by
        # SuperText includes a standard HYPERLINK field around the custom link
        # group so external editors recognize it; only the field result should
        # be displayed when that RTF is pasted back into SuperText.
        if self.hasDirectRTFCommand(structure, 'fldinst'):
            return None
    ...
    def hyperlinkRTFGroupPrefix(self, link, standard_hyperlink=False):
        ...
        if not standard_hyperlink:
            return custom_prefix

        target = self.clipboardHyperlinkTarget(link)
        escaped_target = self.escapeRTFText(target).replace('"', r'\"')
        return (
    ...
    def hyperlinkRTFGroupSuffix(self, standard_hyperlink=False):
        return '}}}}' if standard_hyperlink else '}}'
        ...
        for token_type, token_value, _ in textContents:
            ...
            if token_type == 'tagon' and token_value in self.hyperlink_tags:
                if token_value not in active_hyperlink_tags:
                    ...
                    body += self.hyperlinkRTFGroupSuffix(
                        standard_hyperlinks,
                    ) * len(active_hyperlink_tags)
                    active_hyperlink_tags.clear()
                    body += self.hyperlinkRTFGroupPrefix(
            ...
            if token_type == 'tagoff' and token_value in active_hyperlink_tags:
                ...
                active_hyperlink_tags.remove(token_value)
                body += self.hyperlinkRTFGroupSuffix(standard_hyperlinks)
                continue
    ...
    def convertToRTF(self, start, finish, standard_hyperlinks=False):
    ...
    def copyFromClipboard(self, event=None):
        ...
        if len(text_in_selection) > 0:
            ...
            if any(
                tag in self.hyperlink_tags
                for token_type, tag, _ in selected_text
                if token_type in {'tagon', 'tagoff'}
            ) or any(
                tag in self.hyperlink_tags
                for tag in self.text.tag_names(sel_start)
            ):
                html_format = self.clip.register_format('HTML Format')
                if html_format is not None:
                    fragment = self.selectionAsHTML(selected_text, sel_start)
                    self.clip.set_clipboard(
                        self.clipboardHTMLPayload(fragment),
                        html_format,
                    )
